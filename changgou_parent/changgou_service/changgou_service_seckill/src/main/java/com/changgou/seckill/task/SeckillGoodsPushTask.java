...
public class SeckillGoodsPushTask {
    ...
    public void loadSecKillGoodsToRedis(){
        ...
        List<Date> dateMenus = DateUtil.getDateMenus();//5个时间段

        for (Date dateMenu : dateMenus) {
            SimpleDateFormat simpleDateFormat=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

            String redisExitName = DateUtil.date2Str(dateMenu);
            ...
            criteria.andLessThanOrEqualTo("endTime",simpleDateFormat.format(DateUtil.addDateHour(dateMenu,2)));

            Set keys = redisTemplate.boundHashOps(SECKILL_GOODS_KEY + redisExitName).keys();//key field value
            ...
            for (SeckillGoods seckillGoods : seckillGoodsList) {
                ...
                redisTemplate.opsForHash().put(SECKILL_GOODS_KEY + redisExitName,seckillGoods.getId(),seckillGoods);
